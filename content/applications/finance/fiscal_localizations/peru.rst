...
Guia de Remision Remitente 2.0
------------------------------

The Guía de Remisión Electrónica (Delivery Guide) is an electronic document generated by the 
shipper to support the transportation or transfer of goods from one place to another, such 
as a warehouse or establishment. In Odoo, there are several steps needed in configurations 
before you can successfully use this feature.

This guide is meant to be an all encompassing reference regarding this feature.

Its use is mandatory and required by SUNAT for taxpayers who need to transfer their products, 
with the exception of those under the Single Simplified Regime (Régimen Único Simplificado, or RUS).

Delivery Guide Types
********************

Sender's Delivery Guide
^^^^^^^^^^^^^^^^^^^^^^^

- Issued by the *owner* of the goods (i.e. the sender) at the *beginning* of the shipment.  
- This type of Delivery Guide is issued when a sale is made, a service is rendered (including processing), 
goods are assigned for use or goods are transferred between premises of the same company and others. 
- Learn more `here <https://www.gob.pe/7899-guia-de-remision>`_.
- Included in Odoo.

Carrier’s Delivery Guide
^^^^^^^^^^^^^^^^^^^^^^^^

- It is issued by the *carrier* and must be issued by *each shipper* when the 
*shipment is by public transport*. 
- While the Sender’s Delivery Guide is a document that justifies the shipment of products 
(and issued by the sender), the Carrier Delivery Guide justifies the transportation service performed by the driver (or carrier). 
- Learn more `here <https://tefacturo.pe/blog/sunat/guia-de-remision-electronica/guia-de-remision-transportista/>`_. 
- *Not included in Odoo*.
...
This option is used when the transfer of **goods is carried out by the owner themself** with 
their own vehicles. For this case, a Sender’s Delivery Guide must be issued. 

Public
^^^^^^

This option is used when the **goods are moved by an external carrier**. In this case, 
two Delivery Guides must be issued; the Sender’s Delivery Guide and the Carrier’s Delivery Guide.
...
The creation of the GRE (Delivery Guide) in Odoo **must be sent directly to the SUNAT** regardless 
of the electronic document provider: IAP, Digiflow or SUNAT.

New Required Information
^^^^^^^^^^^^^^^^^^^^^^^^

Version 2.0 requires additional information in the general configuration as well as vehicles, contacts, 
and products. In the general configurations, it will be necessary to add new credentials that 
are acquired in the SUNAT portal.

Flow
****

.. image:: peru/peru_flow_remision.png
   :align: center 
...
Currently, the SUNAT no longer uses the term *“Anula”*, but now uses the term *“Dar de baja”*. 
Therefore, **both** the sender and the carrier can cancel the electronic waybill, *so long as 
the following conditions are met*: 

- If the shipment has not yet been initiated
- If the shipment has been initiated, but the receiver must be changed before reaching 
the final destination.

Testing
^^^^^^^

The SUNAT did not release any test environment; **this means that any Delivery Guides that 
were generated by mistake will be sent to the SUNAT**.

If by mistake the Waybill was created in this environment, it will be necessary to be deleted 
...
.. important::
   - Electronic Sender’s Waybill is currently the only supported type of Waybill in Odoo.
   - The delivery guide is dependant on the **Inventory App**, the `l10n_pe_edi` and 
`l10n_pe` modules.
   
After following the steps to configure the :doc:`Electronic invoicing <accounting_settings>` 
and the :doc:`Master data <configura_master_data>`, go to **Apps**, **clear** any default 
filters set, search for the `l10n_pe_edi_stock_20` module and click the *Activate* button.

Next, you will need to *obtain* the Client ID and Client Secret. To do this, follow the 
instructions in this `link <https://cpe.sunat.gob.pe/sites/default/files/inline-files/Manual_Servicios_GRE.pdf>`_.

.. important::
   Keep in mind, you must generate a second user for electronic document creation of 
electronic documents.
  
To obtain the Client ID / Secret, enter the **SUNAT Portal**, and look in the “SOL” 
menu for the **SUNAT API Credentials Management** section:

.. image:: peru/menu_api_sunat.png
   :align: center 
   :alt: SUNAT API credential management

Make sure you select the checkbox related to the **GRE issuance of receipts** and 
the **Desktop** option in the *scope* section and save the setting:

.. image:: peru/configuration_api_sunat.png
   :align: center 
   :alt: API Credentials registration

.. important::
   Make sure you save and accept the changes so the credentials are updated on 
the SUNAT portal

.. image:: peru/configuration_api_sunat_confirmation.png
   :align: center 
   :alt: API Credentials registration confirmation

These credentials should be used to configure the delivery guide general 
settings following :menuselection:`Accounting --> Configuration --> Settings --> Peruvian Electronic Invoicing` 
and filling the *Sunat Delivery Guide API* section:

.. image:: peru/sunat_delivery_guide_settings.png
   :align: center 
   :alt: Flow for Guia de Remision emission

.. note::
   In the SUNAT portal, it is **important to have the correct access rights 
enabled** as it may differ from the user set in the first section for Electronic Invoicing.

.. warning::
   For the field **Guide SOL User** It is required to follow the format 
‘RUC + UsuarioSol’ (depending on the user selected when generating the 
GRE API Credentials in the SUNAT portal). For example: `20557912879SOLUSER`

.. image:: peru/GRE_fields_example.png
   :align: center 
   :alt: Example for the SUNAT Delivery Guide API section configuration

Operator, Carrier, and Vehicle Configuration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The *Operator* is the driver of the vehicle in the case that the delivery 
guide is done with a *private transport*. 

To create a new operator go to :menuselection:`Contacts --> Create` and 
fill out the contact information, specifically:

- Select *Individual* as the **company type** option.
- Add the *Operator License* in the **Accounting tab** in the contact form.

In the customer address, make sure you complete the following:

- The District field 
- Tax ID (DNI/RUC)
- Tax ID Number

.. image:: peru/operator_configuration.png
   :align: center 
   :alt: Individual type operator configurations in the Contact form

The *Carrier* is used in the case where the delivery guide will be 
done using *public transport*.

To create a new operator go to :menuselection:`Contacts --> Create` 
and fill out the contact information, specifically:

- *Company* selected as the **company type** option
- MTC Registration Number
- Authorization Issuing Entity
- Authorization Number

In the company address, make sure you complete the following:

- The District field 
- Tax ID (DNI/RUC)
- Tax ID Number

.. image:: peru/company_operator_configuration.png
   :align: center 
   :alt:  Company type operator configurations in the Contact form

To configure your available *Vehicles* go to :menuselection:`Inventory App --> Configuration --> Vehicles`, 
it is important to check the `Is M1 or L?` if the vehicle has less than 4 wheels or less than 8 seats.

.. image:: peru/vehicle_m1_or_L_pe.png
   :align: center 
   :alt: Vehicle selected as an M1 or L type
   
If that is not the case, be sure to fulfill the rest of the information 
needed for the vehicle.

.. image:: peru/vehicle_not_M1_or_L_pe.png
   :align: center 
...
configured. The *“Partida Arancelaria” (Tariff Item)* the field needs to be filled. 

To do this, go to :menuselection:`Inventory App --> Products` open the 
product to be configured and fill the “Partida Arancelaria” field.

Common Errors
*************

- `Diferente prefijo para productos (T001 en algunos, T002 en otros)`: At the moment,
Odoo does not support the automation of prefixes for products. This can be done 
manually for each product output. This can be done as well for non-storable products, 
however keep in mind that there will be no traceability.

- `2325 - GrossWeightMeasure - El dato no cumple con el formato establecido "Hace 
falta el campo" "Peso"" en el producto`: This error occurs when the weight on the 
product is set as “0.00”. To fix, you will need to cancel the Waybill and recreate 
it. Make sure that you fix the weight on the product before creating the new 
Waybill, or it will throw you the same error.

- `JSONDecodeError: Expecting value: line 1 column 1 (char 0) when creating a Delivery 
Guide`: This error is typically generated due to SOL user issues. Verify the user’s 
connection with the SUNAT; the SOL user must be established with the company RUT + user ID.
Example: 2012188549JOHNSMITH

- `El número de documento relacionado al traslado de mercancía no cumple con el formato 
establecido : error: documento relacionado: The “Related Document Type” and “Related Document Number”
fields only apply to Invoices and Receipts.

- `400 Client error: Bad Request for URL`: This error is not solvable on Odoo’s side; 
it is advised you reach out to the SUNAT and verify the user. It may be necessary to 
create a new user.

- `Invalid content was found starting with element 'cac:BuyerCustomerParty'`: This 
error occurs when the transfer reason is set as “other”. Please select another option. 
According to the official documentation of the SUNAT’s Waybill Guide, the transfer
reason 03 (sale with shipment to third party) or 12 (others) will not work correctly
in Odoo, since you should not put an “empty” or “blank” customer.

- `Duda cliente: consumo de créditos IAP al usar GRE 2.0`: For live clients using 
IAP: No credit is consumed (in theory) because it does not go through the OSE, i.e.
these documents are directly sent to the SUNAT.

- `Errores con formato credenciales GRE 2.0 (traceback error)`: Odoo currently throws
an error with a traceback, instead of a message that the credentials are not correctly
configured in the database. If this occurs on your database, please verify your credentials.
